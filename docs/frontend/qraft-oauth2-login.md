...
| 入口 | 位置 | 说明 |
| ---- | ---- | ---- |
| 登录门（启动必经，#1095） | 未登录时的启动页（协议门之后） | 未登录不进主界面：只能登录或「退出应用」（先给明确提示再退出）。已登录用户启动直接进主界面 |
...
上次登录存储的环境（无存储时默认**生产环境**；存量测试环境登录态在加载时
自动并入生产，见 `QraftStore.load()` 的 `migrateStoredState`）。
