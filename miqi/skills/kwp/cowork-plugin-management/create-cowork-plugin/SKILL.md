---
name: kwp-cowork-plugin-management-create-cowork-plugin
description: >
metadata: {"miqi": {"requires": {}, "source": "knowledge-work-plugins"}}
---
...
When sharing is needed, use generic language and mark these as requiring customization with two tilde characters such as `create an issue in No project tracker connected — use `write_file` and `edit_file` to maintain local project files, or `exec` to run CLI tools`.
...
| --------------- | ------------------- | ------------------------------- |
| Chat            | `No chat platform connected — use the current conversation or the `message` tool if configured`            | Slack, Microsoft Teams, Discord |
| Project tracker | `No project tracker connected — use `write_file` and `edit_file` to maintain local project files, or `exec` to run CLI tools` | Linear, Asana, Jira             |
```

### the current workspace skills directory Variable

Use `the current workspace skills directory` for all intra-plugin path references in hooks and MCP configs. Never hardcode absolute paths.
...
- **Hooks** config goes in `hooks/hooks.json`. Use `the current workspace skills directory` for script paths. Prefer prompt-based hooks for complex logic.
- **MCP configs** go in `.mcp.json` at plugin root. Use `the current workspace skills directory` for local server paths. Document required env vars in README.
...
- **Portability**: Always use `the current workspace skills directory` for intra-plugin paths, never hardcoded paths.
