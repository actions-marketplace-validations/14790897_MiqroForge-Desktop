---
name: kwp-customer-support-customer-escalation
...
- **No support platform connected — use `write_file` and `edit_file` to track tickets locally**: Related tickets, timeline of communications, previous troubleshooting
- **No CRM connected — use `web_search` and `web_fetch` for company research, or manually enter contact/account details** (if connected): Account details, key contacts, previous escalations
- **No chat platform connected — use the current conversation or the `message` tool if configured**: Internal discussions about this issue, similar reports from other customers
- **No project tracker connected — use `write_file` and `edit_file` to maintain local project files, or `exec` to run CLI tools** (if connected): Related bug reports or feature requests, engineering status
- **No knowledge base connected — use `web_search` and `read_file` to find documentation**: Known issues or workarounds, relevant documentation
...
- "Want me to post this in a No chat platform connected — use the current conversation or the `message` tool if configured channel for the target team?"
