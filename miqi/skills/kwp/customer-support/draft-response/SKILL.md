---
name: kwp-customer-support-draft-response
...
**No email connected — compose emails in text format using `write_file` or respond directly in chat:**
...
**No chat platform connected — use the current conversation or the `message` tool if configured:**
...
**No CRM connected — use `web_search` and `web_fetch` for company research, or manually enter contact/account details (if connected):**
- Account details and plan level
- Contact information and key stakeholders
- Previous escalations or sensitive issues

**No support platform connected — use `write_file` and `edit_file` to track tickets locally (if connected):**
- Related tickets and their resolution
- Known issues or workarounds
- SLA status and response time commitments

**No knowledge base connected — use `web_search` and `read_file` to find documentation:**
