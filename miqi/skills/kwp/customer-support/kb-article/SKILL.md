---
name: kwp-customer-support-kb-article
...
- **No support platform connected — use `write_file` and `edit_file` to track tickets locally**: Pull the ticket thread, resolution, and any internal notes
- **No knowledge base connected — use `web_search` and `read_file` to find documentation**: Check if a similar article already exists (update vs. create new)
- **No project tracker connected — use `write_file` and `edit_file` to maintain local project files, or `exec` to run CLI tools**: Check if there's a related bug or feature request
...
- "Want me to check if a similar article already exists in your No knowledge base connected — use `web_search` and `read_file` to find documentation?"
