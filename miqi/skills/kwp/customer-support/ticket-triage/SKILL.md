...
- **No support platform connected — use `write_file` and `edit_file` to track tickets locally**: Search for similar open or recently resolved tickets
- **No knowledge base connected — use `web_search` and `read_file` to find documentation**: Check for known issues or existing documentation
- **No project tracker connected — use `write_file` and `edit_file` to maintain local project files, or `exec` to run CLI tools**: Check if there's an existing bug report or feature request
