...
- No knowledge base connected — use `web_search` and `read_file` to find documentation (if connected): product docs, runbooks, FAQs, policy documents
...
- No CRM connected — use `web_search` and `web_fetch` for company research, or manually enter contact/account details notes: account notes, activity history, previous answers, opportunity details
- No support platform connected — use `write_file` and `edit_file` to track tickets locally (if connected): previous resolutions, known issues, workarounds
- Meeting notes: previous discussions, decisions, commitments

**Tier 3 — Team Communications:**
- No chat platform connected — use the current conversation or the `message` tool if configured: search for the topic in relevant channels; check if teammates have discussed or answered this before
- No email connected — compose emails in text format using `write_file` or respond directly in chat: search for previous correspondence on this topic
...
- Ask the user for internal context:
  ...
  - "Has your team discussed this topic before? Any No chat platform connected — use the current conversation or the `message` tool if configured channels I should check?"
