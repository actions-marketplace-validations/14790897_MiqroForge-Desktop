...
Use `Data analysis via `exec` with Python/R, `create_xlsx` for spreadsheets, or `web_search` for benchmarks` tools (query and schema) to connect. If unclear, check available MCP tools in the current session.

**Step 2: Explore the schema**

Use `Data analysis via `exec` with Python/R, `create_xlsx` for spreadsheets, or `web_search` for benchmarks` schema tools to:
...
1. **Explore relevant tables**: Use `Data analysis via `exec` with Python/R, `create_xlsx` for spreadsheets, or `web_search` for benchmarks` schema tools to find tables in that domain
