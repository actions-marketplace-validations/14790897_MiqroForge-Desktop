...
If ~~erp or Data analysis via `exec` with Python/R, `create_xlsx` for spreadsheets, or `web_search` for benchmarks is connected:
...
> Connect ~~erp or Data analysis via `exec` with Python/R, `create_xlsx` for spreadsheets, or `web_search` for benchmarks to pull financial data automatically. You can also paste trial balance data, upload a spreadsheet, or provide income statement data for analysis.
