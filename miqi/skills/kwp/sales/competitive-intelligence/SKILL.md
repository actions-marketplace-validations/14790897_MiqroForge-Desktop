...
- **-----------**: -------------- (requires MCP setup — works standalone without it)
- **CRM**: Win/loss history against each competitor, deal-level competitor tracking (requires MCP setup — works standalone without it)
- **Docs**: Existing battlecards, product comparison docs, competitive playbooks (requires MCP setup — works standalone without it)
- **Chat**: Internal chat intel (e.g. Slack) — what your team is hearing from the field (requires MCP setup — works standalone without it)
- **Transcripts**: Competitor mentions in customer calls, objections raised (requires MCP setup — works standalone without it)
