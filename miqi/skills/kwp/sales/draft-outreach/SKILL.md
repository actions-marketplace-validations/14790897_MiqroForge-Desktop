...
- **Enrichment**: Verified email, phone, background details (requires MCP setup — works standalone without it)
- **CRM**: Prior relationship context, existing contacts (requires MCP setup — works standalone without it)
- **Email**: Create draft directly in your inbox (requires MCP setup — works standalone without it)
...
[Draft created - check No email connected — compose emails in text format using `write_file` or respond directly in chat]
